...
void CParticleSystemDefinition::Precache()
{
	...
	m_bIsPrecached = true;
#ifndef DEDICATED
	if ( /*!UTIL_IsDedicatedServer() &&  */g_pMaterialSystem ) // we already know it's not a dedicated server (in the p2 sdk)
	{
		const char* name = m_Name;// MaterialName();
...
void CParticleSystemDefinition::Read( CDmxElement *pElement )
{
	...
	pElement->UnpackIntoStructure2( this, s_pParticleSystemDefinitionUnpack );
...
bool CParticleSystemMgr::ReadParticleDefinitions( CUtlBuffer &buf, const char *pFileName, bool bPrecache, bool bDecommitTempMemory )
{
	...
	for ( int i = 0; i < nCount; ++i )
	{
		...
		if ( pDef && bPrecache )
		{
			...
			if ( m_bAllowPrecache )
			{
				const char* tname = pDef->m_Name;
				tname = pDef->MaterialName();
				pDef->Precache();
			}
		}
