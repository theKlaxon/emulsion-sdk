														   ...
														   float flOODt, ParticleRenderData_t const *pSortList ) const
{
	...
	{
		// Vert0
		meshBuilder.Position3f(0.0f, 1.0f, 0.0f );
		meshBuilder.Color4ub( rc, gc, bc, ac );
		meshBuilder.TexCoord4f( 0, p0.x, p0.y, p0.z, rad );
		meshBuilder.TexCoord4f( 1, p1.x, p1.y, p1.z, rad );
		meshBuilder.TexCoord4f( 2, p2.x, p2.y, p2.z, rad );
		meshBuilder.TexCoord4f( 3, p3.x, p3.y, p3.z, rad );
		meshBuilder.TexCoord4f( 4, pSample0->m_fLeft_U0, pSample0->m_fTop_V0, pSample0->m_fRight_U0, pSample0->m_fBottom_V0 );
		meshBuilder.TexCoord4fv( 5, vFadeColor.Base() );
		
		// Vert1
		meshBuilder.Position3f( 0.0f, 1.0f, 1.0f );
		meshBuilder.Color4ub( rc, gc, bc, ac );
		meshBuilder.TexCoord4f( 0, p0.x, p0.y, p0.z, rad );
		meshBuilder.TexCoord4f( 1, p1.x, p1.y, p1.z, rad );
		meshBuilder.TexCoord4f( 2, p2.x, p2.y, p2.z, rad );
		...
		// Vert2
		meshBuilder.Position3f( 1.0f, 0.0f, 1.0f );
		meshBuilder.Color4ub( rc, gc, bc, ac );
		meshBuilder.TexCoord4f( 0, p0.x, p0.y, p0.z, rad );
		meshBuilder.TexCoord4f( 1, p1.x, p1.y, p1.z, rad );
		meshBuilder.TexCoord4f( 2, p2.x, p2.y, p2.z, rad );
		...
		// Vert3
		meshBuilder.Position3f( 1.0f, 0.0f, 0.0f );
		meshBuilder.Color4ub( rc, gc, bc, ac );
		meshBuilder.TexCoord4f( 0, p0.x, p0.y, p0.z, rad );
		meshBuilder.TexCoord4f( 1, p1.x, p1.y, p1.z, rad );
		meshBuilder.TexCoord4f( 2, p2.x, p2.y, p2.z, rad );
...
BEGIN_PARTICLE_OPERATOR_UNPACK(C_OP_RenderBlobs)
...
//typedef SweepRenderer RENDERER_CLASS;
typedef SweepRenderer RENDERER_CLASS;
...
void C_OP_RenderBlobs::Render(IMatRenderContext* pRenderContext, CParticleCollection* pParticles, const Vector4D& vecDiffuseModulation, void* pContext, int nViewRecursionDepth) const
{
...
#endif
#if 0
	RENDERER_CLASS::setCalcCornerFunc(0, calcCornerNormal);
	...
	tiler->beginFrame(Point3D(0.0f, 0.0f, 0.0f), (void*)&pRenderContext, true); // idk what the third one is so i just said 'true'
	//tiler->beginFrame(Point3D(pParticles->m_Center), (void*)&pRenderContext, true); // idk what the third one is so i just said 'true'
	...
	for (int i = 0; i < nParticles; i++)
	{
		...
		//float x = pParticles->m_Center.x;
		//float y = pParticles->m_Center.y;
		//float z = pParticles->m_Center.z;

		ImpParticle* imp_particle = &imp_particles_sa[i];
		imp_particle->center.set(x, y, z);
		tiler->insertParticle(imp_particle);
	}

	//Point3D p = Point3D();
	//tiler->drawSurfaceSorted(p);
...
class C_OP_RenderModels : public CParticleRenderOperatorInstance
{
	...
	uint32 GetReadAttributes( void ) const
	{
		return PARTICLE_ATTRIBUTE_XYZ_MASK | PARTICLE_ATTRIBUTE_SEQUENCE_NUMBER_MASK | PARTICLE_ATTRIBUTE_TINT_RGB_MASK | PARTICLE_ATTRIBUTE_ALPHA_MASK | PARTICLE_ATTRIBUTE_ALPHA2_MASK | PARTICLE_ATTRIBUTE_RADIUS_MASK |
			   PARTICLE_ATTRIBUTE_ROTATION_MASK | PARTICLE_ATTRIBUTE_YAW_MASK | PARTICLE_ATTRIBUTE_NORMAL_MASK | 1 << m_nAnimationScaleField;
	}
...
void C_OP_RenderModels::Precache( void )
{
	...
	if (V_strcmp(m_ActivityName, ""))
		m_nActivity = g_pParticleSystemMgr->Query()->GetActivityCount();// (m_pModels[0], m_ActivityName);
	else
		m_nActivity = -1;
}
...
void C_OP_RenderModels::Render( IMatRenderContext *pRenderContext, CParticleCollection *pParticles, const Vector4D &vecDiffuseModulation, void *pContext, int nViewRecursionDepth ) const
{
	...
	for( int i = 0; i < nNumParticles; i++ )
	{
		...
		g_pParticleSystemMgr->Query()->DrawModel( m_pModels[ 0 ], matFinal, pParticles, hParticle, nSequence, 1, m_nSkin, nAnimationSequence, flAnimationRate, r, g, b );
	}
