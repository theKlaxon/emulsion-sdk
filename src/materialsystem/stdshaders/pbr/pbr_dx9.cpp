...
BEGIN_BASE_SHADER(PBR, "PBR shader")
    ...
    SHADER_INIT
    {
        ...
        int envMapFlags = g_pHardwareConfig->GetHDRType() == HDR_TYPE_NONE ? TEXTUREFLAGS_PRE_SRGB : 0;// TEXTUREFLAGS_SRGB : 0;
    ...
    SHADER_DRAW
    {
        ...
        if (IsSnapshotting())
        {
            ...
            {
                // theres a naming error or smth with one of the files. will sort another time
                // Setting up static vertex shader
                DECLARE_STATIC_VERTEX_SHADER(pbr_vs20); // was pbr_vs20b
                SET_STATIC_VERTEX_SHADER(pbr_vs20);     // was pbr_vs20b
