...
#include "pbr_paint_ps30.inc"
//#include "pbr_paint_ps20b.inc"
...
void DrawPbrPass_Paint(CBaseVSShader* pShader, IMaterialVar** params, IShaderDynamicAPI* pShaderAPI, IShaderShadow* pShaderShadow,
	...
	if (pShader->IsSnapshotting())
	{
		...
		// paint
		pShaderShadow->EnableTexture(SAMPLER_SPLATNORMAL, true);
		pShaderShadow->EnableSRGBRead(SAMPLER_SPLATNORMAL, false);

		pShaderShadow->EnableTexture(SAMPLER_PAINT, true);
		pShaderShadow->EnableSRGBRead(SAMPLER_PAINT, false);
		...
		//else
		{
			...
			// Setting up static pixel shader
			DECLARE_STATIC_PIXEL_SHADER(pbr_paint_ps30);
			SET_STATIC_PIXEL_SHADER_COMBO(FLASHLIGHT, switches.bHasFlashlight);
			SET_STATIC_PIXEL_SHADER_COMBO(FLASHLIGHTDEPTHFILTERMODE, switches.nShadowFilterMode);
			SET_STATIC_PIXEL_SHADER_COMBO(LIGHTMAPPED, switches.bLightMapped);
			...
			SET_STATIC_PIXEL_SHADER_COMBO(PARALLAXCORRECT, switches.bHasParallaxCorrection);
			SET_STATIC_PIXEL_SHADER(pbr_paint_ps30);
		}
	...
	else // Not snapshotting -- begin dynamic state
	{
		...
		{
			pShader->BindTexture(SAMPLER_NORMAL, info.normalTexture, 0);
		}
		else
		{
		...
		//else
		{
			...
			// Setting up dynamic pixel shader
			DECLARE_DYNAMIC_PIXEL_SHADER(pbr_paint_ps30);
			SET_DYNAMIC_PIXEL_SHADER_COMBO(NUM_LIGHTS, lightState.m_nNumLights);
			SET_DYNAMIC_PIXEL_SHADER_COMBO(WRITEWATERFOGTODESTALPHA, bWriteWaterFogToAlpha);
			SET_DYNAMIC_PIXEL_SHADER_COMBO(WRITE_DEPTH_TO_DESTALPHA, bWriteDepthToAlpha);
			SET_DYNAMIC_PIXEL_SHADER_COMBO(PIXELFOGTYPE, pShaderAPI->GetPixelFogCombo());
			SET_DYNAMIC_PIXEL_SHADER_COMBO(FLASHLIGHTSHADOWS, bFlashlightShadows);
			SET_DYNAMIC_PIXEL_SHADER(pbr_paint_ps30);
		}
