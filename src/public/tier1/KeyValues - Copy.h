...
public:
	...
	bool LoadFromFileEX(IBaseFileSystem* filesystem, const char* resourceName, const char* pathID = NULL, GetSymbolProc_t pfnEvaluateSymbolProc = NULL); // this wil override the original

#if defined (CLIENT_DLL) || defined(GAME_DLL) || defined(EMULSION_DLL)
#define LoadFromFile LoadFromFileEX
	...
	uint64 GetUint64(int keySymbol, uint64 defaultValue = 0); // asw
	...
	bool GetBool(int keySymbol, bool defaultValue = false) { return GetInt(keySymbol, defaultValue ? 1 : 0) ? true : false; } // asw
	...
	static KeyValues* FromString(char const* szName, char const* szStringVal, char const** ppEndOfParse = NULL); // TODO: port from asw???
...
//	void WriteConvertedString( CUtlBuffer &buffer, const char *pszString );
	...
	void SaveKeyToFile( KeyValues *dat, IBaseFileSystem *filesystem, FileHandle_t f, CUtlBuffer *pBuf, int indentLevel);
