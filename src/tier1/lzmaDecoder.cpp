...
//-----------------------------------------------------------------------------
// Specifically tailored to vcs shader files
// 
// Uncompress a buffer, Returns the uncompressed size. Caller must provide an
// adequate sized output buffer or memory corruption will occur.
//-----------------------------------------------------------------------------
unsigned int CLZMA::UncompressVCS(unsigned char* pInput, unsigned char* pOutput, LZMAReadProgressCallbackFunc_t pCallback)
{
	lzma_header_t* pHeader = (lzma_header_t*)pInput;

	unsigned int actualSize = pHeader->actualSize;// GetActualSize(pInput);
	if (!actualSize)
	{
		// unrecognized
		return 0;
