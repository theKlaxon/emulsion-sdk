...
//	cbParallaxMap->setChecked( true );
	...
	mxCheckBox *cbPhysicsModel = new mxCheckBox (wRender, 275, 5, 150, 20, "Physics Model", IDC_PHYSICSMODEL);
...
void ControlPanel::SetupSequenceWindow( mxTab* pTab )
{
	...
	mxCheckBox *cbBlendSequences = new mxCheckBox (wSequence, 5, 142, 150, 20, "Blend Sequence Changes", IDC_BLENDSEQUENCECHANGES);
	mxButton *btnBlendNow = new mxButton( wSequence, 155, 142, 80, 20, "Blend Now", IDC_BLENDNOW );
	...
	if (hdr)
	{
		...
		for (int i = 0; i < MAX_SEQUENCES; i++)
		{
			...
			for (int j = 0; j < hdr->GetNumSeq(); j++)
			{
				...
				Assert( nSequence < ARRAYSIZE(iSequenceToSelection) );
