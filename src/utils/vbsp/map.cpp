...
ChunkFileResult_t LoadDispMultiBlendKeyCallback( const char *szKey, const char *szValue, mapdispinfo_t *pMapDispInfo )
{
	if ( !strnicmp( szKey, "row", 3 ) )
	{
		...
		while ( pszNext != NULL )
		{
			...
			vMultiBlend.x = V_atof( pszNext );
			pszNext = strtok( NULL, " " );
			vMultiBlend.y = V_atof( pszNext );
			pszNext = strtok( NULL, " " );
			vMultiBlend.z = V_atof( pszNext );
			pszNext = strtok( NULL, " " );
			vMultiBlend.w = V_atof( pszNext );
			pszNext = strtok( NULL, " " );

			pMapDispInfo->multiBlends[nIndex].m_vMultiBlend = vMultiBlend;
...
ChunkFileResult_t LoadDispAlphaBlendKeyCallback( const char *szKey, const char *szValue, mapdispinfo_t *pMapDispInfo )
{
	if ( !V_strnicmp( szKey, "row", 3 ) )
	{
		...
		while ( pszNext != NULL )
		{
			...
			vAlphaBlend.x = V_atof( pszNext );
			pszNext = strtok( NULL, " " );
			vAlphaBlend.y = V_atof( pszNext );
			pszNext = strtok( NULL, " " );
			vAlphaBlend.z = V_atof( pszNext );
			pszNext = strtok( NULL, " " );
			vAlphaBlend.w = V_atof( pszNext );
			pszNext = strtok( NULL, " " );

			pMapDispInfo->multiBlends[nIndex].m_vAlphaBlend = vAlphaBlend;
...
static int s_nMultiBlendColorIndex = 0;

ChunkFileResult_t LoadDispMultiBlendColorCallback0( CChunkFile *pFile, mapdispinfo_t *pMapDispInfo )
{
	s_nMultiBlendColorIndex = 0;
...
ChunkFileResult_t LoadDispMultiBlendColorKeyCallback( const char *szKey, const char *szValue, mapdispinfo_t *pMapDispInfo )
{
	if ( !V_strnicmp( szKey, "row", 3 ) )
	{
		...
		while ( pszNext != NULL )
		{
			...
			color.x = V_atof( pszNext );
			pszNext = strtok( NULL, " " );
			color.y = V_atof( pszNext );
			pszNext = strtok( NULL, " " );
			color.z = V_atof( pszNext );
			pszNext = strtok( NULL, " " );

			pMapDispInfo->multiBlends[nIndex].m_vMultiBlendColors[s_nMultiBlendColorIndex] = color;
...
ChunkFileResult_t CMapFile::LoadEntityCallback(CChunkFile *pFile, int nParam)
{
	...
	if (eResult == ChunkFile_Ok)
	{
		...
		if ( alldetail && !V_strcmp( pClassName, "func_brush" ) && !V_strncmp( ValueForKey( mapent, "targetname" ), "structure_", 10 ) )
		{
	...
	for ( int i = 0; i < num_entities; i++ )
	{
		...
		if ( !strcmp( pEntity, "func_instance" ) )
		{
			...
			if ( pInstanceFile[ 0 ] )
			{
				...
				if ( bLoaded == false )
				{
					Error( "Could not open instance file %s\n", pInstanceFile );
				}
			}
...
ChunkFileResult_t CMapFile::LoadSideCallback(CChunkFile *pFile, LoadSide_t *pSideInfo)
{
	...
	if (eResult == ChunkFile_Ok)
	{
		...
		if ( alldetail && pSideInfo->pEntity == &entities[0] )
		{
			side->contents |= CONTENTS_DETAIL;
		}
