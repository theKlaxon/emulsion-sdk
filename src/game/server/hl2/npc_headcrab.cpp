...
void CBaseHeadcrab::HandleAnimEvent( animevent_t *pEvent )
{
	...
	if ((pEvent->type & AE_TYPE_NEWEVENTSYSTEM) && (pEvent->type & AE_TYPE_SERVER))
	{
		...
		else if (nEvent == AE_HEADCRAB_CEILING_DETACH)
		{
			SetMoveType(MOVETYPE_STEP);
		...
		else if (nEvent == AE_HEADCRAB_JUMP_TELEGRAPH)
		{
			TelegraphSound();
		...
		else if (nEvent == AE_HEADCRAB_BURROW_IN)
		{
		...
		else if (nEvent == AE_HEADCRAB_BURROW_IN_FINISH)
		{
			SetBurrowed(true);
			return;
		}
		else if (nEvent == AE_HEADCRAB_BURROW_OUT)
		{
			Assert(m_bBurrowed);
			if (m_bBurrowed)
			{
