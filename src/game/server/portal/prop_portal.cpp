...
#include "..\game\shared\portal\portal_placement.h"
...
CProp_Portal::CProp_Portal( void )
{
	if( !ms_DefaultPortalSizeInitialized )
	{
		...
		//CEG_GCV_PRE();
		ms_DefaultPortalHalfHeight = PORTAL_HALF_HEIGHT;// CEG_GET_CONSTANT_VALUE(DefaultPortalHalfHeight); // only protecting one to reduce the cost of first-portal check
		//CEG_GCV_POST();
	}
...
void CProp_Portal::CreatePortalEffect( CBasePlayer* pPlayer, int iEffect, Vector vecOrigin, QAngle qAngles, int nTeam, int nPortalNum )
{
	...
	//if ( pPlayer->m_bPredictionEnabled ) // TODO: check if this matters
	//{
...
extern PortalPlacementResult_t VerifyPortalPlacementAndFizzleBlockingPortals(const CProp_Portal* pIgnorePortal, Vector& vOrigin, QAngle& qAngles, float fHalfWidth, float fHalfHeight, PortalPlacedBy_t ePlacedBy);
extern bool PortalPlacementSucceeded(PortalPlacementResult_t eResult);
