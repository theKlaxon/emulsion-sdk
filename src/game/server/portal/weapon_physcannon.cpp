...
bool CGrabController::IsObjectAllowedOverhead( CBaseEntity *pEntity )
{
	...
	//if( UTIL_IsCombineBallDefinite(pEntity) ) // TODO: maybe add energy ball back in?
	//	return true;
...
{
	return false;// (HL2GameRules()->MegaPhyscannonActive() == true);
}
...
bool CWeaponPhysCannon::AttachObject( CBaseEntity *pObject, const Vector &vPosition )
{
	...
	if( pOwner )
	{
		...
		{
			pOwner->FlashlightTurnOff(false);
		}
#endif
	...
	if( pOwner )
	{
		...
		float	maxSpeed = pl_normspeed.GetFloat() + ( (pl_normspeed.GetFloat() - pl_normspeed.GetFloat() ) * loadWeight );
...
void CWeaponPhysCannon::DetachObject( bool playSound, bool wasLaunched )
{
	...
	{
		//pOwner->EnableSprint( true );
		pOwner->SetMaxSpeed( pl_normspeed.GetFloat() );
