...
public:
	...
	//-------------------------------------------------------------------------
	virtual void Spawn();
	virtual void VPhysicsCollision( int index, gamevcollisionevent_t *pEvent, IPhysicsObject *pOtherObject );
...
template< typename BasePropType >
void PropPaintPowerUser<BasePropType>::VPhysicsCollision( int index, gamevcollisionevent_t *pEvent, IPhysicsObject *pOtherObject )
{
	...
	BaseClass::VPhysicsCollision( index, pEvent, pOtherObject );
}
...
{
	static int s_SpeedMaterialIndex = GetPhysicsSurfaceProps()->GetSurfaceIndex( "ice" );
