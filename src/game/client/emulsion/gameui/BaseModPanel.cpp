	...
	m_lastActiveUserId( 0 )
{
	...
	m_UIScheme = vgui::scheme()->LoadSchemeFromFileEx( 0, "resource/emulsionscheme.res", "EmulsionScheme" );
	...
	m_backgroundMusic = "Music.MainMenu";
...
CBaseModFrame* CBaseModPanel::OpenWindow(const WINDOW_TYPE & wt, CBaseModFrame * caller, bool hidePrevious, KeyValues *pParameters)
{
	...
	if ( GetActiveWindowPriority() > newNav->GetWindowPriority() )
	{
		...
		newNav->SetVisible( false ); // was disabled
	}
	else
	{
...
void CBaseModPanel::OnGameUIActivated()
{
	...
	else if ( ( !m_LevelLoading && !engine->IsConnected() ) || GameUI().IsInBackgroundLevel() )
	{
		...
		if ((!IsPC() || bForceReturnToFrontScreen))// && !GameUI().IsOverridingBaseModPanel() )
		{
			// this is now done in cdll_client_int.cpp CHLClient::PostInit() to be compatible with the gamepadui flow
			//OpenFrontScreen();
		}
	}
