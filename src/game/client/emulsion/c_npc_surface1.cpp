...
#include "blobulator/implicit/ImpDefines.h"
...
LINK_ENTITY_TO_CLASS(npc_surface, C_NPC_Surface);
...
int C_NPC_Surface::DrawModel( int flags, const RenderableInstance_t& instance)
{
	...
	if( !IsErrorMaterial( m_pMaterial ) )
	{
		...
		else
		{
			...
			// we must use tiler in p2 (at least with this version of the headers)
