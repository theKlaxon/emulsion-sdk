...
#if defined( SWARM_DLL )// || defined (P2_DLL)
...
#include "portal2/gameui/portal2/basemodui.h"
typedef BaseModUI::CBaseModPanel UI_BASEMOD_PANEL_CLASS;
inline UI_BASEMOD_PANEL_CLASS& GetUiBaseModPanelClass() { return UI_BASEMOD_PANEL_CLASS::GetSingleton(); }
inline UI_BASEMOD_PANEL_CLASS& ConstructUiBaseModPanelClass() { return *new UI_BASEMOD_PANEL_CLASS(); }
IMatchExtPortal2 g_MatchExtPortal2;
class IMatchExtPortal2* g_pMatchExt = &g_MatchExtPortal2;
#endif
...
//bool CGameUI::UpdateSecondaryProgressBar(float progress, const wchar_t* desc) { return false; }
//
...
bool CGameUI::LoadingProgressWantsIsolatedRender(bool bContextValid) { return false; }
bool CGameUI::IsPlayingFullScreenVideo() { return false; }
bool CGameUI::IsTransitionEffectEnabled() { return false; }
	...
	if ( IsPC() && m_bTryingToLoadFriends && m_iFriendsLoadPauseFrames-- < 1 && g_hMutex && g_hWaitMutex )
	{
		...
		if (waitResult == SYS_WAIT_OBJECT_0 || waitResult == SYS_WAIT_ABANDONED)
		{
			...
			if ( pathSep )
			{
				...
				if ( modinfo->LoadFromFileEX( g_pFullFileSystem, "gameinfo.txt" ) )
				{
