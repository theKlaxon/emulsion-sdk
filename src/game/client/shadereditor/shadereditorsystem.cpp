...
#ifdef EMULSION_DLL
	Q_snprintf(modulePath, sizeof(modulePath), "%s/bin/shadereditor_portal2.dll", engine->GetGameDirectory());
#elif SWARM_DLL
	...
	void DrawOpaqueRenderables_Custom( bool bShadowDepth )
	{
		...
		if( r_drawothermodels.GetBool() )
		{
			for ( int i = 0; i < numOpaqueEnts; ++i )
			{
				...
				IClientModelRenderable* pModelRenderable;

#ifdef EMULSION_DLL
				pModelRenderable = itEntity->m_pRenderable->GetClientModelRenderable();
#else
				pModelRenderable = pUnknown->GetClientModelRenderable();
#endif
...
#ifdef EMULSION_DLL
	render->Push3DView( pRenderContext, viewModelSetup, iClearFlags, pTex, pCView->GetFrustum() );
#else
...
#ifdef EMULSION_DLL
	render->PopView( pRenderContext, pCView->GetFrustum() );
#else
