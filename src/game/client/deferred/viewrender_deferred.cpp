...
void CDeferredViewRender::DrawWorldComposite( const CViewSetup &view, int nClearFlags, bool bDrawSkybox )
{
	...
	//render->GetVisibleFogVolume( view.origin, &fogVolumeInfo );
	render->GetVisibleFogVolume( view.origin, NULL, &fogVolumeInfo ); // p2
...
void CDeferredViewRender::DrawViewModels( const CViewSetup &view, bool drawViewmodel, bool bGBuffer )
{
	...
	render->Push3DView( pRenderContext, viewModelSetup, 0, NULL, GetFrustum() ); // p2 wanted render context
...
void CDeferredViewRender::RenderView( const CViewSetup &view, const CViewSetup &hudViewSetup, int nClearFlags, int whatToDraw )
{
	...
	if ( true )
	{
		...
		render->Push2DView( pRenderContext, hudViewSetup, 0, saveRenderTarget, GetFrustum() ); // p2
		...
		render->PopView( pRenderContext, GetFrustum() ); // p2
	}
	pRenderContext.SafeRelease();

	//FlushWorldLists();
	extern void FlushWorldCache();
	FlushWorldCache();
...
void CBaseWorldViewDeferred::DrawOpaqueRenderablesDeferred( bool bNoDecals )
{
	...
	if( r_drawothermodels.GetBool() )
	{
		for ( int i = 0; i < numOpaqueEnts; ++i )
		{
			...
			IClientModelRenderable* pModelRenderable = itEntity->m_pRenderable->GetClientModelRenderable();// pUnknown->GetClientModelRenderable(); // p2
	...
	CMatRenderContextPtr pRenderContext(materials);
	g_pParticleSystemMgr->DrawRenderCache( /*pRenderContext,*/ false );
	pRenderContext.SafeRelease();
}
...
{
	CMatRenderContextPtr pRenderContext(materials);
	render->Push3DView( pRenderContext, *this, 0, m_pDummyTexture, GetFrustum(), m_pDepthTexture );
