...
void ClientModeSDK::OnColorCorrectionWeightsReset( void )
{
	...
	if ( m_CCDeathHandle != INVALID_CLIENT_CCHANDLE)
	{
		m_flDeathCCWeight = Approach( pPlayer->IsAlive() ? 0.0f : 0.0f, m_flDeathCCWeight, gpGlobals->frametime * ( 1.0f / DEATH_CC_FADE_SPEED ) );
		...
		if ( m_flDeathCCWeight != 0.0f && m_pCurrentColorCorrection )
		{
