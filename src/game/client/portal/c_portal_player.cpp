...
int C_Portal_Player::DrawModel( int flags, const RenderableInstance_t& instance)
{
	...
	if( IsLocalPlayer() )
	{
		...
		{
			if ( !GetPortalRender().IsRenderingPortal() )
				return 0;

			if( (GetPortalRender().GetViewRecursionLevel() == 1) && (m_iForceNoDrawInPortalSurface != -1) ) //CPortalRender::s_iRenderingPortalView )
				return 0;
		}
	}
...
void C_Portal_Player::PlayerPortalled( C_Prop_Portal *pEnteredPortal )
{
	if( pEnteredPortal )
	{
		...
		if( IsLocalPlayer() )
			GetPortalRender().EnteredPortal( pEnteredPortal );
	}
}
