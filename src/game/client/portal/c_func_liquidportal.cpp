...
{
	GetPortalRender().AddPortal( this );
}

C_Func_LiquidPortal::~C_Func_LiquidPortal( void )
{
	GetPortalRender().RemovePortal( this );
}
...
{
	DrawOutwardBox( GetPortalRender().m_MaterialsAccess.m_WriteZ_Model );
	DrawInnerLiquid( true, 1.0f, GetPortalRender().m_MaterialsAccess.m_WriteZ_Model );
}
...
void CPortalRenderable_Func_LiquidPortal::RenderPortalViewToBackBuffer( CViewRender *pViewRender, const CViewSetup &cameraView )
{
...

	{
		...
		render->Push3DView( pRenderContext, portalView, 0, NULL, pViewRender->GetFrustum() );		
		{
			...
			SetViewRecursionLevel( GetPortalRender().GetViewRecursionLevel() + 1 );

			CPortalRenderable *pRenderingViewForPortalBackup = GetPortalRender().GetCurrentViewEntryPortal();
			CPortalRenderable *pRenderingViewExitPortalBackup = GetPortalRender().GetCurrentViewExitPortal();
