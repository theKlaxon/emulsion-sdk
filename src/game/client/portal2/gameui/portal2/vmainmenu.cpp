...
#include "..\common\CegClientWrapper.h"
...
CPlaySPSelectStorageDevice:: CPlaySPSelectStorageDevice () :
	CChangeStorageDevice( XBX_GetActiveUserId() )
{
...
#endif
		{
			BASEMODPANEL_SINGLETON.ViewAllCommunityMapsInWorkshop();
			//MsgOpenSinglePlayer();
		}
	}
...
/*CEG_NOINLINE */void MainMenu::ApplySchemeSettings( IScheme *pScheme )
{
	...
	if ( !g_pMatchFramework->GetMatchSystem()->GetPlayerManager()->GetLocalPlayer( XBX_GetActiveUserId() ) )
	{
...
void MainMenu::MsgOpenSinglePlayer()
{
	...
	//
	DWORD dwDevice = XBX_GetStorageDeviceId( XBX_GetActiveUserId() );
