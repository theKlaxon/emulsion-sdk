...
#include "paint_player_shared.h"
#include "c_paint_player.h"
...
private:
	...
	vgui::ImagePanel *m_pPaintImage[PAINT_POWER_TYPE_COUNT];
	float m_fImageTimers[ PAINT_POWER_TYPE_COUNT ];
		...
		: CHudElement( pElementName ), EditablePanel( NULL, "PaintIcon" )
{
	...
	m_pPaintImage[BOUNCE_POWER] = new vgui::ImagePanel( this, "PaintBounceImage" );
	m_pPaintImage[STICK_POWER] = new vgui::ImagePanel( this, "PaintStickImage" );
	m_pPaintImage[SPEED_POWER] = new vgui::ImagePanel( this, "PaintSpeedImage" );
...
void CHUDPaintIcons::OnTick( void )
{
	...
	C_Paint_Player *pPaintPlayer = ToPaintPlayer( C_BasePlayer::GetLocalPlayer() );

	if( pPaintPlayer )
	{
		const C_PaintPlayerLocalData& localData = pPaintPlayer->GetPaintPlayerLocalData();
