...
int CNewParticleEffect::DrawModel( int flags, const RenderableInstance_t &instance )
{
	...
	// TODO: implement a proper view recursion if adding portals in
	int nViewRecursion = 0;
	...
	if ( ( flags & STUDIO_TRANSPARENCY ) || !IsBatchable() || !m_pDef->IsDrawnThroughLeafSystem() )
	{
		...
		Render( nViewRecursion, pRenderContext, vecDiffuseModulation, ( flags & STUDIO_TRANSPARENCY ) ? IsTwoPass() : false, pCameraObject );
		pRenderContext->MatrixMode( MATERIAL_MODEL );
	...
	Msg("RendererCount for %s: %i\n", GetName(), GetRendererCount());
