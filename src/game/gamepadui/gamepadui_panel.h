...
using namespace SchemeValueTypes;// = SchemeValueTypes::SchemeValueType;
...
public:
    ...
    void AddValueToSchemeMap( const char *pszName, const char *pszScriptName, SchemeValueType Type, char const *pszDefaultValue, PANELLOOKUPFUNC pfnFunc)
    {
        ...
        m_Values.AddToTail( { pszName, pszScriptName, Type, pszDefaultValue, pfnFunc } );
    }

    void UpdateSchemeProperties( vgui::Panel* pPanel, vgui::IScheme* pScheme )
    {
        //for ( SchemeValue& value : m_Values)
        //for ( SchemeValue& value : CUtlVector<SchemeValue>::)
        //for(SchemeValue* value = m_Values.Base(); value != &m_Values.Tail(); value)
        for (int i = 0; i < m_Values.Count(); i++)
        {
            SchemeValue& value = m_Values[i];
