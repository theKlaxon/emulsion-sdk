...
protected:
    ...
    GAMEPADUI_PANEL_PROPERTY( Color, m_colBinding, "Button.Binding", "0 0 0 255", SchemeValueTypes::Colour);
};
...
private:
    ...
    GAMEPADUI_BUTTON_ANIMATED_PROPERTY( Color, m_colSliderBacking, "Slider.Backing", "255 255 255 22", SchemeValueTypes::Colour);
    GAMEPADUI_BUTTON_ANIMATED_PROPERTY( Color, m_colSliderFill, "Slider.Fill", "255 255 255 255", SchemeValueTypes::Colour);
...
private:
    ...
    GAMEPADUI_BUTTON_ANIMATED_PROPERTY( Color, m_colImage, "Button.Image", "255 255 255 255",  SchemeValueTypes::Colour);
    GAMEPADUI_BUTTON_ANIMATED_PROPERTY( Color, m_colTitleBackground, "Button.Title.Background", "0 0 0 0",  SchemeValueTypes::Colour);
    GAMEPADUI_BUTTON_ANIMATED_PROPERTY( Color, m_colDescriptionBackground, "Button.Description.Background", "0 0 0 0",  SchemeValueTypes::Colour);
};
...
void GamepadUIOptionsPanel::LayoutCurrentTab()
{
    ...
    for ( int i = 0; i < m_nTabCount; i++ )
    {
        ...
        for ( int i = 0; i < tab.pButtons.Count(); i++)
            tab.pButtons[i]->SetVisible(false);
    ...
    for (int i = 0; i < m_Tabs[nActiveTab].pButtons.Count(); i++)
    {
        auto pButton = m_Tabs[nActiveTab].pButtons[i];
        int fade = 255;
        ...
        if ( pCvarButton )
        {
            ...
            if ( pszDependentCVar && *pszDependentCVar )
            {
                ...
                for (int i = 0; i < m_Tabs[nActiveTab].pButtons.Count(); i++)
                {
                    auto pOtherButton = m_Tabs[nActiveTab].pButtons[i];
...
void GamepadUIOptionsPanel::ClearBindings()
{
    for ( int i = 0; i < m_nTabCount; i++ )
    {
        ...
        for(int j = 0; m_Tabs[i].pButtons.Count(); j++)
        {
            auto pButton = m_Tabs[i].pButtons[j];
...
void GamepadUIOptionsPanel::FillInBindings()
{
    ...
    for ( int i = 0; i < BUTTON_CODE_LAST; i++ )
	{
        ...
        for ( int j = 0; j < m_nTabCount; j++ )
        {
            ...
            for (int k = 0; k < m_Tabs[j].pButtons.Count(); k++)
            {
                auto pFindButton = m_Tabs[j].pButtons[k];
		...
		if ( pButton )
		{
            ...
            if ( *pCurrentKey )
            {
				...
				if( !bShouldOverride && bFalcon /*&& IsNovintCode((ButtonCode_t)i) && !IsNovintCode(currentBC)*/ ) // TODO: figure out IsNovintCode
...
void GamepadUIOptionsPanel::ApplyKeyBindings()
{
    ...
    for (int i = 0; i < m_Tabs[nTab].pButtons.Count(); i++)
    {
        auto pButton = m_Tabs[nTab].pButtons[i];
        GamepadUIKeyButton *pKeyButton = dynamic_cast< GamepadUIKeyButton* >( pButton );
        if ( !pKeyButton )
            continue;
...
void GamepadUIOptionsPanel::OnKeyBound( const char *pKey )
{
    for ( int i = 0; i < m_nTabCount; i++ )
    {
        ...
        {
            auto pFindButton = m_Tabs[i].pButtons[j];
            GamepadUIKeyButton* pFindKeyButton = dynamic_cast<GamepadUIKeyButton*>(pFindButton);
