...
void GamepadUIBasePanel::OnMenuStateChanged()
{
    ...
    SetBackgroundMovieEnabled(GamepadUI::GetInstance().IsInLevel() ? false : true);
...
void GamepadUIBasePanel::SetBackgroundMovie(const char* szMovieName) 
{
    m_pMovie->SetMovie(szMovieName);
}

void GamepadUIBasePanel::SetBackgroundMovieEnabled(bool bEnabled)
{
    m_bBackgroundMovieEnabled = bEnabled;
    SetPaintBackgroundEnabled(false);
}

void GamepadUIBasePanel::PaintBackgroundMovie() {
    if (m_bBackgroundMovieEnabled)
        m_pMovie->Update();
}