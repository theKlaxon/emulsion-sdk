	...
	, m_bIsSave( bIsSave )
{
	...
	if ( m_bIsSave )
		m_Saves.AddToTail( { "NewSavedGame", "", "", "#GameUI_NewSaveGame", "", "", "Current", NEW_SAVE_GAME_TIMESTAMP } );
...
void GamepadUISaveGamePanel::SaveGame( const SaveGameDescription_t* pSave )
{
	...
	if ( saveName && saveName[0] )
	{
		...
		GamepadUI::GetInstance().GetEngineClient()->ClientCmd_Unrestricted( sz );
		Close();
		// TODO: outdated in p2, find a replacement in necessary (Klax)
