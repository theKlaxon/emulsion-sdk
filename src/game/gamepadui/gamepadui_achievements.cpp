...
public:
    ...
    void Paint() OVERRIDE
    {
        ...
        if ( m_nGoal > 1 )
        {
            ...
            int nLength = swprintf_s( wcsProgress, L"%d/%d", m_nCount, m_nGoal );
...
private:
    ...
    GAMEPADUI_PANEL_PROPERTY( Color, m_colProgressColor,   "Button.Background.Progress",   "255 0 0 255", SchemeValueTypes::Colour);
    GAMEPADUI_PANEL_PROPERTY( Color, m_colUnprogressColor, "Button.Background.Unprogress", "255 0 0 255", SchemeValueTypes::Colour);
...
GamepadUIAchievementsPanel::GamepadUIAchievementsPanel( vgui::Panel *pParent, const char* pPanelName ) : BaseClass( pParent, pPanelName )
{
    ...
    {
        IAchievement *pCurAchievement = GamepadUI::GetInstance().GetAchievementMgr()->GetAchievementByIndex( i, 0 );
        if ( !pCurAchievement )
            continue;
        ...
        sprintf_s( szIconName, "gamepadui/achievements/%s%s.vmt", pCurAchievement->GetName(), pCurAchievement->IsAchieved() ? "" : "_bw" );
        //sprintf_s

        char szMaterialName[MAX_PATH];
        sprintf_s( szMaterialName, "materials/%s", szIconName );
        if ( !g_pFullFileSystem->FileExists( szMaterialName ) )
            sprintf_s( szIconName, "vgui/achievements/%s%s.vmt", pCurAchievement->GetName(), pCurAchievement->IsAchieved() ? "" : "_bw" );

        sprintf_s( szMaterialName, "materials/%s", szIconName );
        if ( !g_pFullFileSystem->FileExists( szMaterialName ) )
            strcpy_s( szIconName, "vgui/hud/icon_locked.vmt" );
