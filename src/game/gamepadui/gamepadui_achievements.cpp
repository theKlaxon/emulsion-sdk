...
public:
    ...
    void Paint() override
    {
        ...
        if ( m_nGoal > 1 )
        {
            ...
            int nLength = swprintf( wcsProgress, L"%d/%d", m_nCount, m_nGoal );
...
GamepadUIAchievementsPanel::GamepadUIAchievementsPanel( vgui::Panel *pParent, const char* pPanelName ) : BaseClass( pParent, pPanelName )
{
    ...
    {
        IAchievement *pCurAchievement = GamepadUI::GetInstance().GetAchievementMgr()->GetAchievementByIndex( i, 0 ); // TODO: proper splitscreen handling (Klaxon)
        if ( !pCurAchievement )
            continue;
